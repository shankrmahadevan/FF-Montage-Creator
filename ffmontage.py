        ...
        try:
          ...
          if current_frame_no%divisor==0:
            ...
            if flag:
              ...
              subprocess.run([f'ffmpeg -ss {start_time} -i temp/download.mp4 -vf drawtext="fontfile={ttf_path}: text={text_dis}: fontcolor=white: fontsize=15: x=0: y=0" -ss 0 -c copy -t {time_interval*2} -c:v libx264 -crf 26 -preset superfast -avoid_negative_ts make_zero -tune film temp/to_concat/{vid_no}.mp4'], shell=True)
              last_end = end_time
